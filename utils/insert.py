...
def toProcess(lost):
  # This all still needs to be adjusted manually. . .
  ...
  for l in lost:
    ...
    resultname = '-'.join([parts[3], parts[4], parts[5], parts[6]]).replace('.zip','') + '-preprocess-' + '-'.join([parts[0],parts[1],parts[2]])
...
## SETUP
blockname = '4D'
blockid = '5994d5f755f30b1586146396'
sessionname = '4d2'
...
preprocess = s3.list_objects(Bucket=config.get('s3', 'bucket'), Prefix=s3prefix + '/preprocess-frames')['Contents']
preprocess = [os.path.basename(p['Key']) for p in preprocess]

detection = s3.list_objects(Bucket=config.get('s3', 'bucket'), Prefix=s3prefix + '/detection')['Contents']
detection = [os.path.basename(d['Key']) for d in detection if '.zip' in d['Key']]

process = s3.list_objects(Bucket=config.get('s3', 'bucket'), Prefix=s3prefix + '/process-frames')['Contents']
...
# a = [re.match('2017[^p]+',obj['Key'].split('/')[-1]).group()[:-1]+'.tar.gz' for obj in  s3.list_objects(Bucket=config.get('s3', 'bucket'), Prefix='{}/results/farm_{}/block_{}/{}/preprocess-frames'.format(clientid, farmname, blockname, '4d'))['Contents']]
# b = [re.match('2017[^p]+',obj['Key'].split('/')[-1]).group()[:-1]+'.tar.gz' for obj in  s3.list_objects(Bucket=config.get('s3', 'bucket'), Prefix='{}/results/farm_{}/block_{}/{}/preprocess-frames'.format(clientid, farmname, blockname, '4d2'))['Contents']]
# tarfiles = set(a) - set(b)

# Preprocess diff rows/directions
# a = [obj['Key'].split('/')[-1] for obj in s3.list_objects(Bucket=config.get('s3', 'bucket'), Prefix='{}/results/farm_{}/block_{}/{}/preprocess-frames'.format(clientid, farmname, blockname, '4d'))['Contents']]
# b = [obj['Key'].split('/')[-1] for obj in s3.list_objects(Bucket=config.get('s3', 'bucket'), Prefix='{}/results/farm_{}/block_{}/{}/preprocess-frames'.format(clientid, farmname, blockname, '4d2'))['Contents']]
